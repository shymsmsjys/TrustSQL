...
bool tldgr_trust_options_precheck(THD *thd, const LEX_CSTRING *table_name, List<Create_field> &create_fields, KEY **key_info, uint *key_count)
{
	...
	CLOG_FUNCTIOND("static bool tldgr_const4t_precheck(...)");

	// Check the sig_column_name is in the create_fields.
	while(sfield=sfi_it++) {		
		target_found=false;
	...
	// if TRUSTED ORDERED Table, there should be only one TRUSTED ORDER Constraints. (Check sfield->sig_field_type ) nothing also fail!
	...
	// Common, mandatory
	if(trust_options->Table_issuer_pubk.str==0) { my_error(5006,MYF(0)); goto err; }
	if(trust_options->Table_image.str==0) { my_error(5007,MYF(0)); goto err; }
	if(trust_options->Table_issuer_tableimage_sign.str==0) { my_error(5008,MYF(0)); goto err; }

	if(thd->lex->create_info.is_Trusted_Ordered()) {
		if(trust_options->Trusted_orderer_master_pubk.str==0) { my_error(5009,MYF(0)); goto err; }
	//	if(trust_options->Trusted_orderer_sub_pubk->str==0) { my_error(5010,MYF(0)); goto err; }
	}

	// Table Sign Check
	if(verify_string(thd, trust_options->Table_image, trust_options->Table_issuer_pubk, trust_options->Table_issuer_tableimage_sign)) {
		my_error(5011,MYF(0));
		goto err;
	}
...
LEX_CUSTRING tldgr_const4t_build_image(LEX_trust_options *trust_options, List<Create_field> &create_fields, KEY **key_info, uint *key_count) {
	...
	uchar tldHeader[TLDGR_TLC_HEADER_SIZE];
	uchar *frm_ptr, *pos;
	...
	bzero((char*)tldHeader,TLDGR_TLC_HEADER_SIZE);

	tldHeader[0]=(uchar) 254;
	tldHeader[1]= 1;
	tldHeader[2]= (uchar)TLEDGER_TLC_VERSION;
	...
	CLOG_TPRINTLN("tldHeader[2]= (0x%08X) -> TLEDGER_TLC_VERSION",tldHeader[2]);
	...
	CLOG_TPRINTLN("tldHeader[8]   ~    L(2)V(N)= TI_PUB, TO_M, TO_S, TI_SIGN");
   	int2store(pos,trust_options->Table_issuer_pubk.length);
	pos+=2;
	memcpy(pos,trust_options->Table_issuer_pubk.str,trust_options->Table_issuer_pubk.length);
	pos+=trust_options->Table_issuer_pubk.length;
	CLOG_TPRINTLN("TABLE_ISSUER_PUB_KEY=%s",trust_options->Table_issuer_pubk.str);
	
	int2store(pos,trust_options->Trusted_orderer_master_pubk.length);
	pos+=2;
	memcpy(pos,trust_options->Trusted_orderer_master_pubk.str,trust_options->Trusted_orderer_master_pubk.length);
	pos+=trust_options->Trusted_orderer_master_pubk.length;
	CLOG_TPRINTLN("Trusted Orderer Master PUB_KEY=%s",trust_options->Trusted_orderer_master_pubk.str);

	int2store(pos,trust_options->Trusted_orderer_sub_pubk.length);
	pos+=2;
	memcpy(pos,trust_options->Trusted_orderer_sub_pubk.str,trust_options->Trusted_orderer_sub_pubk.length);
	pos+=trust_options->Trusted_orderer_sub_pubk.length;
	CLOG_TPRINTLN("Trusted Orderer Sub PUB_KEY=%s",trust_options->Trusted_orderer_sub_pubk.str);	

	int2store(pos,trust_options->Table_issuer_tableimage_sign.length);
	pos+=2;
	memcpy(pos,trust_options->Table_issuer_tableimage_sign.str,trust_options->Table_issuer_tableimage_sign.length);
	pos+=trust_options->Table_issuer_tableimage_sign.length;
	CLOG_TPRINTLN("Table Issuer Table Image Sign=%s",trust_options->Table_issuer_tableimage_sign.str);	

	int2store(pos,trust_options->Table_child_pubk.length);
	pos+=2;
	memcpy(pos,trust_options->Table_child_pubk.str,trust_options->Table_child_pubk.length);
	pos+=trust_options->Table_child_pubk.length;
	CLOG_TPRINTLN("Table Child PUB_KEY=%s",trust_options->Table_child_pubk.str);	
	...
	*/
	...
	while(sfield=sfi_it++) {
		...
		if(sfield->verification_key_type==FIXED_KEY) {
			...
			if(sfield->sig_field_type==sign_only_field) 
				extra_size+=trust_options->Table_issuer_pubk.length ; // 
			else if(sfield->sig_field_type==sign_ordered_field) 
				extra_size+=trust_options->Trusted_orderer_master_pubk.length ; // 		
	...
	int2store(tldHeader+(TLDGR_TLC_HEADER_SIZE-2),trust_options->Table_image.length+2+extra_size);

	frm.length = TLDGR_TLC_HEADER_SIZE + trust_options->Table_image.length + 2 + extra_size;
	...
	memcpy(frm_ptr, tldHeader, TLDGR_TLC_HEADER_SIZE);

	CLOG_STEP("5","build extra image");
	pos = frm_ptr + TLDGR_TLC_HEADER_SIZE;
	int2store(pos,trust_options->Table_image.length);
	pos+=2;
	memcpy(pos,trust_options->Table_image.str,trust_options->Table_image.length);
	pos+=trust_options->Table_image.length;
	CLOG_TPRINTLN("Table Image=%s",trust_options->Table_image.str);
...
bool init_share_from_tld_image(THD *thd, TABLE_SHARE*share,const uchar *tld_image, size_t tld_length) {
	...
	if(len!=0) {
		...
		temp_str[len]=0;
		pos+=len;
		share->trust_options->Table_issuer_pubk.str=temp_str;
		share->trust_options->Table_issuer_pubk.length=len;
	}
	...
	if(len!=0) {
		...
		temp_str[len]=0;
		pos+=len;
		share->trust_options->Trusted_orderer_master_pubk.str=temp_str;
		share->trust_options->Trusted_orderer_master_pubk.length=len;
	}
	...
	if(len!=0) {
		...
		temp_str[len]=0;
		pos+=len;
		share->trust_options->Trusted_orderer_sub_pubk.str=temp_str;
		share->trust_options->Trusted_orderer_sub_pubk.length=len;
	}
	...
	if(len!=0) {
		...
		temp_str[len]=0;
		pos+=len;
		share->trust_options->Table_issuer_tableimage_sign.str=temp_str;
		share->trust_options->Table_issuer_tableimage_sign.length=len;
	}
	...
	if(len!=0) {
		...
		temp_str[len]=0;
		pos+=len;
		share->trust_options->Table_child_pubk.str=temp_str;
		share->trust_options->Table_child_pubk.length=len;
	}

	sig_field_pos = &tld_image[TLDGR_TLC_HEADER_SIZE];
    ...
    if(len!=0) {
    	...
    	temp_str[len]=0;
    	sig_field_pos+=len;
    	share->trust_options->Table_image.str=temp_str;
    	share->trust_options->Table_image.length=len;
    }
...
bool tldgr_add_const4t_table(THD *thd, const char *frm, size_t frm_length, const char *path, const char *db, const char *table_name){
    ...
    strxnmov(file_name, sizeof(file_name)-1, path, TLDGR_CNF_FILE_EXT, NullS);
	...
	for (val= *option_list; val; val= val->next)
	{
	  ...
	  if (!my_strnncoll(system_charset_info,
			  ...
			  (uchar*)val->name.str, val->name.length)) {
				  thd->lex->trust_options->Trusted_orderer_master_pubk = val->value;
...
bool trusted_table_exists(THD* thd, const LEX_CSTRING *db, const LEX_CSTRING *table_name) {
	...
	if (file_ext_exists(path, path_len, TLDGR_CNF_FILE_EXT))
	{
		return true;
	}
	return false;
...
int check_table_definition_trusted(THD *thd, LEX_CSTRING *db_name, LEX_CSTRING *table_name, LEX_CSTRING child_issuer_key) {
	...
	uint len;
	char path[FN_REFLEN + 1];
	uchar tldgr_head[TLDGR_TLC_HEADER_SIZE+2]; // Header + Table Image Len
	uchar *file_buffer;
	const char *pos;
	char *temp_str;
	uint errorno;
	...
	path_len = build_table_filename(path, sizeof(path) - 1,
											 db_name->str, table_name->str, TLDGR_CNF_FILE_EXT, 0);
	...
	len = uint2korr(&tldgr_head[TLDGR_TLC_HEADER_SIZE]);
