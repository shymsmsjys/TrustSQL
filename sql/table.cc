...
enum open_frm_error open_table_def(THD *thd, TABLE_SHARE *share, uint flags)
{
  ...
  uchar *tldgr_image;
  uchar tldgr_head[TLDGR_TLC_HEADER_SIZE];
#endif
  ...
  if (share->trusted_table_type!=0) {
	  ...
	  length=(uint) (strxmov(path, share->normalized_path.str, TLDGR_CNF_FILE_EXT, NullS) -
	                  path);
	  ...
	  frmlen= TLDGR_TLC_HEADER_SIZE+uint2korr(tldgr_head+(TLDGR_TLC_HEADER_SIZE-2));
                                            ...
                                            size_t frm_length)
{
  ...
  if(frm_image[59]=='T') {
	  CLOG_TPRINTLN("Original Firm Len=%d",frm_length);
  	  CLOG_TPRINTLN("Trusted Firm Len=%d",frm_length + TLDGR_TLC_HEADER_SIZE + uint2korr(frm_image+frm_length+TLDGR_TLC_HEADER_SIZE));
  	  if (write && write_frm_image(frm_image, frm_length + TLDGR_TLC_HEADER_SIZE + uint2korr(frm_image+frm_length+TLDGR_TLC_HEADER_SIZE)))
  		  goto err;
  } else {
                        ...
                        HA_CREATE_INFO *create_info, uint keys, KEY *key_info)
{
  ...
  */
  ...
  if(thd->lex->create_info.is_Trusted()) {
	  CLOG_TPRINTLN("!!!!!!!!!!!!!! THIS IS A TRUSTED TABLE , Set FRM[TLDGR_FLAG_OFFSET]=='T'");
	  CLOG_TPRINTLN("!!!!!!!!!!!!!! Please Note it works when it is called CREATE!");
	  fileinfo[TLDGR_FLAG_OFFSET]= 'T';
  }
#endif
