...
{
  int pth_ret;
  CLOG_FUNCTIOND("my_bool my_thread_global_init(void)");
  ...
  */
  CLOG_STEP("1","create THR_KEY_mysys");
  ...
  CLOG_STEP("2","Mutex used by my_thread_init() and after my_thread_destroy_mutex()");
  my_thread_init_internal_mutex();

  CLOG_STEP("3","my_thread_init()");
  if (my_thread_init())
    return 1;
...
my_bool my_thread_init(void)
{
  struct st_my_thread_var *tmp;
  my_bool error=0;
  CLOG_FUNCTIOND("my_bool my_thread_init(void)");
