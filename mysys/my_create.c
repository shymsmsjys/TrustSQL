	       ...
	       myf MyFlags)
{
  ...
  CLOG_FUNCTIOND("File my_create(const char *FileName, int CreateFlags, int access_flags,myf MyFlags)");
  CLOG_TPRINTLN("Create a new file");

  CLOG_STEP("1","Open the file");
#if defined(_WIN32)
  ...
  CLOG_TPRINTLN("FD(0x%08X)",fd);
  CLOG_STEP("2","Regist filename");
