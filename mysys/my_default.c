                  ...
                  int *argc, char ***argv, const char ***default_directories)
{
  ...
  CLOG_FUNCTIOND("int my_load_defaults(const char *conf_file, const char **groups,int *argc, char ***argv, const char ***default_directories)");
  CLOG_TPRINTLN("conf_file = %s",conf_file);

  init_alloc_root(&alloc, "my_load_defaults", 512, 0, MYF(0));
  CLOG_STEP("1","init_default_directories");
  ...
  */
  CLOG_STEP("2","Check if the user doesn't want any default option processing --no-defaults is always the first option");
  ...
  CLOG_STEP("3","my_search_option_files");
...
#if ZLOG_STARTUP_ON==1
  CLOG_TPRINTLN("Option List");
  for(int i=0; i< *argc; i++) {
	  CLOG_TPRINTLN(" OPTION [%d ] = %s",i,res[i]);
  }
#endif
