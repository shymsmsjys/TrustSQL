...
{
  char *str;
  CLOG_FUNCTIOND("my_bool my_init(void)");
  ...
  CLOG_TPRINTLN("Default creation of new files = %o",my_umask);
  /* Default creation of new dir's */
  if ((str= getenv("UMASK_DIR")) != 0)
    my_umask_dir= (int) (atoi_octal(str) | 0700);
  CLOG_TPRINTLN("Default creation of new dir's = %o",my_umask_dir);
  init_glob_errs();
  ...
  CLOG_TPRINTLN("my_progname_shor = %s",my_progname_short);
  /* Initialize our mutex handling */
  CLOG_STEP("1","Initialize our mutex handling");
  my_mutex_init();

  CLOG_STEP("2","Initialize Global Thread");
  ...
  CLOG_TPRINTLN("home_dir = %s",home_dir);
  {
    DBUG_ENTER("my_init");
