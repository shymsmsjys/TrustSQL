...
{
  File fd;
  CLOG_FUNCTIOND("File my_open(const char *FileName, int Flags, myf MyFlags)");
  DBUG_ENTER("my_open");
  DBUG_PRINT("my",("Name: '%s'  Flags: %d  MyFlags: %lu",
		   FileName, Flags, MyFlags));
  CLOG_TPRINTLN("Name: '%s'  Flags: %d  MyFlags: %lu",  FileName, Flags, MyFlags);
  ...
  CLOG_TPRINTLN(" return fd = 0x%08X",fd);
			  ...
			  type_of_file, uint error_message_number, myf MyFlags)
{
  ...
  CLOG_FUNCTIOND("File my_register_filename(File fd, const char *FileName, enum file_type...)");
  CLOG_TPRINTLN("Register file in mu_file_info file name = %s",FileName);
