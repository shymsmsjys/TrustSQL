...
void *my_malloc(size_t size, myf my_flags)
{
  ...
  CLOG_FUNCTIOND("void *my_malloc(size_t size, myf my_flags)");
  CLOG_TPRINTLN("size: %lu  my_flags: %lu", (ulong) size, my_flags);
...
void my_free(void *ptr)
{
  ...
  CLOG_FUNCTIOND("void my_free(void *ptr)");
  CLOG_TPRINTLN("ptr: %p", ptr);
  if (ptr)
  {
    size_t old_size;
    my_bool old_flags;
