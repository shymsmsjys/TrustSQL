                     ...
                     myf my_flags)
{
 ...
 CLOG_FUNCTIOND("void init_alloc_root(MEM_ROOT *mem_root, const char *name, size_t block_size,...");
 CLOG_TPRINTLN("root: %p  name: %s  prealloc: %zu", mem_root,name, pre_alloc_size);
                         ...
                         size_t pre_alloc_size __attribute__((unused)))
{
  ...
  CLOG_FUNCTIOND("void reset_root_defaults(MEM_ROOT *mem_root, size_t block_size,...");
...
void *alloc_root(MEM_ROOT *mem_root, size_t length)
{
  ...
  CLOG_FUNCTIOND("void *alloc_root(MEM_ROOT *mem_root, size_t length)");
  CLOG_TPRINTLN("enter - root: %p  name: %s", mem_root, mem_root->name);
...
//  ZLOG_TPRINTLN("Aligned length = %d", length);
//  ZLOG_TPRINTLN(" free = %d", mem_root->free);
  if ((*(prev= &mem_root->free)) != NULL)
  {
//	ZLOG_TPRINTLN("&mem_root->free is not null  = %p",&mem_root->free);
    if ((*prev)->left < length &&
...
//  ZLOG_TPRINTLN("next =  %p",next);
  if (! next)
  {						/* Time to alloc new block */
    block_size= (mem_root->block_size & ~1) * (mem_root->block_num >> 2);
    get_size= length+ALIGN_SIZE(sizeof(USED_MEM));
    get_size= MY_MAX(get_size, block_size);
//    ZLOG_TPRINTLN(" block_Size = %d", block_size);
//    ZLOG_TPRINTLN(" get_size = %d", get_size);
...
//  ZLOG_TPRINTLN("mem_root->block_num = %d",mem_root->block_num);
//  ZLOG_TPRINTLN("mem_root->total_alloc = %d",mem_root->total_alloc);
//  ZLOG_TPRINTLN("next->next = %p",next->next);
//  ZLOG_TPRINTLN("next->size = %d", next->size);

  point= (uchar*) ((char*) next+ (next->size-next->left));
 // ZLOG_TPRINTLN("point  = %p", point);
...
void *multi_alloc_root(MEM_ROOT *root, ...)
{
  ...
  CLOG_FUNCTIOND("void *multi_alloc_root(MEM_ROOT *root, ...)");
  /*
...
void free_root(MEM_ROOT *root, myf MyFlags)
{
  ...
  CLOG_FUNCTIOND("vvoid free_root(MEM_ROOT *root, myf MyFlags)");
  CLOG_TPRINTLN("root: %p  name: %s  flags: %u", root, root->name, (uint) MyFlags)
